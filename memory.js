...
function createMemoryDeck(cardCount) {
  ...
  if (!selectedSet || selectedSet.values.length < pairCount) {
    updateMemoryStatus('Für dieses Motiv gibt es noch nicht genug Bilder.');
    return [];
  }
...
function handleMemoryCardClick(index) {
  ...
  if (!firstCard) {
    ...
    updateMemoryStatus('Erste Karte aufgedeckt. Wähle eine zweite Karte.');
    return;
  }
...
function markPairAsMatched() {
  ...
  if (matchedPairs === totalPairs) {
    updateMemoryStatus(`Sehr gut! Alle Paare gefunden. Züge: ${moveCount}.`);
    return;
  }
